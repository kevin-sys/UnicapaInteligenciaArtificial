...
{
    public partial class FRMBackpropagationCascada : Form
    {
        Leer l = new Leer();
        public string ARCHIVO = "";

        DatosBackpropagationCascada datosBackpropagationCascada = new DatosBackpropagationCascada();
        DatosBackpropagationCascadaService datosBackpropagationCascadaService = new DatosBackpropagationCascadaService();

        IList<DatosBackpropagationCascada> listaNumeroCapaFuncion = new List<DatosBackpropagationCascada>();
        IList<DatosBackpropagationCascada> listas_W = new List<DatosBackpropagationCascada>();
        IList<DatosBackpropagationCascada> listas_U = new List<DatosBackpropagationCascada>();
        ...
        IList<DatosBackpropagationCascada> Lista_UmbralActualizar = new List<DatosBackpropagationCascada>();
        IList<DatosBackpropagationCascada> Lista_PesosinicialesActualizar = new List<DatosBackpropagationCascada>();
        IList<DatosBackpropagationCascada> Lista_FuncionActivacion = new List<DatosBackpropagationCascada>();
        IList<DatosBackpropagationCascada> Lista_DerivadaFuncionActivacion = new List<DatosBackpropagationCascada>();
        IList<DatosBackpropagationCascada> Lista_Simulacion = new List<DatosBackpropagationCascada>();
        public FRMBackpropagationCascada()
        {
            InitializeComponent();
            Ocultar();
        }
        ...
        private void BotonReiniciar_Click(object sender, EventArgs e)
        {
            ...
            FRMBackpropagationCascada fRMBackpropagationCascada = new FRMBackpropagationCascada();
            fRMBackpropagationCascada.Refresh();
        ...
        private void BotonNumeroNeuronas_Click(object sender, EventArgs e)
        {
            ...
            if (Capas >= Num)
            {
                ...
                datosBackpropagationCascada.NumeroCapas = (Num+1).ToString();
                datosBackpropagationCascada.NumeroNeuronasXCapas = TextoNumeroNeuronasXCapaC.Text;
                datosBackpropagationCascada.FuncionActivacion = ComboFuncionActivacion.Text;

                listaNumeroCapaFuncion.Add(datosBackpropagationCascada);
        ...
        private void BotonRegistrarFunActivaSalida_Click(object sender, EventArgs e)
        {
            ...
            if (Capas >= Num2)
            {
                ...
                datosBackpropagationCascada.NumeroCapas = (Num2+1).ToString();
                datosBackpropagationCascada.FuncionActivacion = ComboFuncionActivacionSalida.Text;
        ...
        private void BotonEntrenar_Click(object sender, EventArgs e)
        {
            do
            {
                ...
                {
                    Res = datosBackpropagationCascadaService.GuardarPesosActualizar(item.ToString(), CargarPasosUmbrales.ToString());
                    Res = datosBackpropagationCascadaService.GuardarPesosIniciales(item.ToString(), CargarPasosUmbrales.ToString());
                }

                foreach (var item in Lista_U)
                {
                    Res = datosBackpropagationCascadaService.GuardarUmbralActualizar(item.ToString(), CargarPasosUmbrales.ToString());
                    datosBackpropagationCascadaService.GuardarUmbral(item.ToString(), CargarPasosUmbrales.ToString());
                }
            ...
            do
            {
                ...
                {
                    ERMS = datosBackpropagationCascada.Calcular_ERMS(lista_EP, Convert.ToInt32(TextoPatronesC.Text));
                    ...
                    else
                    {
                        ...
                        lista_ENi.Clear();
                        lista_ENl.Clear();
                        Res = datosBackpropagationCascadaService.EliminarENi();
                        Res = datosBackpropagationCascadaService.EliminarENl();
                        lista_EP.Clear();
                        iteracion++;
                        m = 0;
                    }
                ...
                else
                {
                    ...
                    {
                        Lista_PesosinicialesActualizar = datosBackpropagationCascadaService.PintarPesoInicialActualizar(p.ToString());
                        Lista_UmbralActualizar = datosBackpropagationCascadaService.PintarUmbralActualizar(p.ToString());

                        Res = datosBackpropagationCascadaService.EliminarArchivoPesoInicialesActualizar(p.ToString());
                        Res = datosBackpropagationCascadaService.EliminarArchivoUmbralActualizar(p.ToString());
                        Res = datosBackpropagationCascadaService.EliminarArchivoFuncionActivacion(p.ToString());
                        Res = datosBackpropagationCascadaService.EliminarArchivoDerivadaFuncionActivacion(p.ToString());
                                ...
                                for (int i = 0; i < Lista_PesosinicialesActualizar.Count; i++)
                                {
                                    for (int t = 0; t < Lista_X.Count; t++)
                                    {
                                        if (i == index && t == index2 && Cont_index <= Lista_X.Count)
                                        {
                                            ...
                                            NumCapa = datosBackpropagationCascada.Capa(X, W, U);
                        ...
                        {
                            Lista_FuncionActivacion = datosBackpropagationCascadaService.PintarFuncionActivacion((p - 1).ToString());
                        ...
                        foreach (var item in FuncionActivacion)
                        {
                            ...
                            {
                                FunActivacion = datosBackpropagationCascada.Identidad(item);
                                DerivadaFunActivacion = datosBackpropagationCascada.Derivada_Identidad(item);
                            }
                            else if (Nombrefunciaon == "ESCALON")
                            {
                                FunActivacion = datosBackpropagationCascada.Escalon(item);
                                DerivadaFunActivacion = datosBackpropagationCascada.Derivada_Escalon(item);
                            }
                            else if (Nombrefunciaon == "LINEA A TRAMOS")
                            {
                                FunActivacion = datosBackpropagationCascada.Lineal_Tramos(item);
                                DerivadaFunActivacion = datosBackpropagationCascada.Derivada_Lineal_Tramos(item);
                            }
                            else if (Nombrefunciaon == "SIGMOIDEA")
                            {
                                FunActivacion = datosBackpropagationCascada.Sigmoidea(Convert.ToDouble(item));
                                DerivadaFunActivacion = datosBackpropagationCascada.Derivada_Sigmoidea(Convert.ToDouble(item));
                            }
                            else if (Nombrefunciaon == "GAUSSIANA")
                            {
                                FunActivacion = datosBackpropagationCascada.Gaussiana(Convert.ToDouble(item));
                                DerivadaFunActivacion = datosBackpropagationCascada.Derivada_Gaussiana(Convert.ToDouble(item));
                            }
                            else if (Nombrefunciaon == "SINUSOIDAL")
                            {
                                FunActivacion = datosBackpropagationCascada.Sinusoidal(Convert.ToDouble(item));
                                DerivadaFunActivacion = datosBackpropagationCascada.Derivada_Sinusoidal(Convert.ToDouble(item));
                            }
                        ...
                        foreach (var item in FuncionActivacion2)
                        {
                            ...
                            Res = datosBackpropagationCascadaService.GuardarFuncionActivacion(item.ToString(), p.ToString());
                        }
                        
                        foreach (var item in DerivadaFuncionActivacion2)
                        {
                            //MessageBox.Show(item.ToString());
                            Res = datosBackpropagationCascadaService.GuardarDerivadaFuncionActivacion(item.ToString() , p.ToString());
                        }
                        ...
                        if (p == (sum1 + sum2))
                        {
                            EL = datosBackpropagationCascada.CalcularEL(YD, FunActivacion);
                            EP = datosBackpropagationCascada.CalcularEP(EL, Convert.ToInt32(TextoSalidaC.Text));
                            lista_EP.Add(EP);
                            ...
                            //no se llena 
                            listas_W.Clear();
                            listas_W = datosBackpropagationCascadaService.ConsultarPesosInicial((sum1 + sum2).ToString());

                            foreach (var item in listas_W)
                            {
                                ENl = datosBackpropagationCascada.Error_EN_l(EL, item.W);
                            ...
                            {
                                Res = datosBackpropagationCascadaService.GuardarENl(item.ToString());
                            }
                            ...
                            listas_W = datosBackpropagationCascadaService.ConsultarPesosInicial(sum1.ToString());
                            ...
                            {
                                Res = datosBackpropagationCascadaService.GuardarENi(item.ToString());
                            }
                            ...
                            for (int t = 1; t <= (sum1 + sum2); t++)
                            {
                                ...
                                Lista_PesosinicialesActualizar = datosBackpropagationCascadaService.ConsultarPesosInicial(t.ToString());
                                Lista_UmbralActualizar = datosBackpropagationCascadaService.ConsultarUmbral(t.ToString());
                                Lista_DerivadaFuncionActivacion = datosBackpropagationCascadaService.PintarDerivadaFuncionActivacion(t.ToString());
                                Lista_FuncionActivacion = datosBackpropagationCascadaService.PintarFuncionActivacion(t.ToString());
                                ...
                                if (t == 1)
                                {
                                            ...
                                            for (int z = 0; z < Lista_DerivadaFuncionActivacion.Count; z++)
                                            {
                                                for (int w = 0; w < Lista_X.Count; w++)
                                                {
                                                    if (q == index && y == index3 && z == index3 && w == index2 && Cont_index <= Lista_X.Count)
                                                    {
                                                        ...
                                                        W_Actualizado = datosBackpropagationCascada.W_Actualizar(W, rata, eni, DEV_Funcion_ACT, X);
                                    ...
                                    for (int y = 0; y < lista_ENi.Count; y++)
                                    {
                                        for (int q = 0; q < Lista_UmbralActualizar.Count; q++)
                                        {
                                            if (y == q)
                                            {
                                                ...
                                                U = Lista_UmbralActualizar[q].U;
                                                eni = lista_ENi[y];
                                                U_Actualizado = datosBackpropagationCascada.U_Actualizar(U, rata, eni);
                                    ...
                                    Res = datosBackpropagationCascadaService.EliminarArchivoPesoInicialesActualizar(t.ToString());
                                    Res = datosBackpropagationCascadaService.EliminarArchivoUmbralActualizar(t.ToString());

                                    foreach (var item in lista_Pesos)
                                    {
                                        Res = datosBackpropagationCascadaService.GuardarPesosActualizar(item.ToString(), t.ToString());
                                    }
                                    
                                    foreach (var item in lista_Umbrales)
                                    {
                                        Res = datosBackpropagationCascadaService.GuardarUmbralActualizar(item.ToString(), t.ToString());
                                    }
                                ...
                                {
                                    Lista_FuncionActivacion.Clear();
                                    Lista_FuncionActivacion = datosBackpropagationCascadaService.PintarFuncionActivacion((t-1).ToString());
                                    ...
                                    for (int q = 0; q < Lista_PesosinicialesActualizar.Count; q++)
                                    {
                                        for (int w = 0; w < Lista_FuncionActivacion.Count; w++)
                                        {
                                            if (q == index && w == index2 && Cont_index <= Lista_FuncionActivacion.Count)
                                            {
                                                ...
                                                W_Actualizado = datosBackpropagationCascada.W3_Actualizar(W, rata, EL, Funcion_ACT);
                                                lista_Pesos.Add(W_Actualizado);
                                                //MessageBox.Show(W_Actualizado.ToString());
                                    ...
                                    for (int q = 0; q < Lista_UmbralActualizar.Count; q++)
                                    {
                                        rata = Convert.ToDecimal(TextoRataAprendizaje.Text);
                                        U = Lista_UmbralActualizar[q].U;
                                        U_Actualizado = datosBackpropagationCascada.U2_Actualizar(U, rata, EL);
                                        //MessageBox.Show(U_Actualizado.ToString());
                                        lista_Umbrales.Add(U_Actualizado);
                                    }

                                    datosBackpropagationCascadaService.EliminarArchivoPesoInicialesActualizar(t.ToString());
                                    datosBackpropagationCascadaService.EliminarArchivoUmbralActualizar(t.ToString());
                                        ...
                                        for (int y = 0; y < lista_ENl.Count; y++)
                                        {
                                            for (int w = 0; w < Lista_DerivadaFuncionActivacion.Count; w++)
                                            {
                                                if (q == index && w == index2 && y==index2 && Cont_index <= Lista_DerivadaFuncionActivacion.Count)
                                                {
                                                    ...
                                                    enl = lista_ENl[y];
                                                    W_Actualizado = datosBackpropagationCascada.W2_Actualizar(W, rata, enl, DEV_Funcion_ACT);
                                                    lista_Pesos.Add(W_Actualizado);
                                                    //MessageBox.Show(W_Actualizado.ToString());
                                                    //MessageBox.Show("["+ W.ToString() +"]-["+ rata.ToString() + "]-["+ EP.ToString() + "]-["+ X.ToString()+ "] "+ (w+1).ToString());
                                                    index++;
                                    ...
                                    for (int y = 0; y < lista_ENl.Count; y++)
                                    {
                                        for (int q = 0; q < Lista_UmbralActualizar.Count; q++)
                                        {
                                            if (y == q)
                                            {
                                                ...
                                                U = Lista_UmbralActualizar[q].U;
                                                enl = lista_ENl[y];
                                                U_Actualizado = datosBackpropagationCascada.U_Actualizar(U, rata, enl);
                                                //MessageBox.Show(U_Actualizado.ToString());
                                                lista_Umbrales.Add(U_Actualizado);
                                            }
                                        }
        ...
        private void BotonSimulacion_Click(object sender, EventArgs e)
        {
            ...
            for (int p = 1; p <= (sum1 + sum2); p++)
            {
                ...
                {
                    //MessageBox.Show(item.ToString());
                    Res = datosBackpropagationCascadaService.GuardarFuncionActivacionSimulacion(item.ToString(), p.ToString());
                }
                ...
                if (p == (sum1 + sum2))
                {
                    ...
                    {
                        Lista_Simulacion = datosBackpropagationCascadaService.PintarFuncionActivacionSimulacion((i + 1).ToString());
